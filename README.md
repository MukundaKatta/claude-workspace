...
A curated cut of the 391 active originals, sorted by theme. The full list lives in [`data/repos.ndjson`](data/repos.ndjson) and is regenerated by `scripts/refresh.sh`.
...
The landing repo is **[bedrock-production-stack](https://github.com/MukundaKatta/bedrock-production-stack)** — story + comparison vs Phoenix/Langfuse/Helicone/LiteLLM + a 50-line runnable example that wires all three together. Mirrors the [agent-stack](https://github.com/MukundaKatta/agent-stack) JS pattern.

| Repo | Stack | Description |
|---|---|---|
| [bedrock-production-stack](https://github.com/MukundaKatta/bedrock-production-stack) | Python | Front-door / landing repo for the three libraries below. Includes a 50-line runnable example that exercises all three. CI green on Python 3.10–3.13. |
| [bedrockcache](https://github.com/MukundaKatta/bedrockcache) | Python | Static auditor that catches "this Bedrock call won't actually cache" before the request goes out. Six backends: bedrock-converse, bedrock-invoke-anthropic, AnthropicBedrock, LiteLLM, Strands, pydantic-ai. CLI + pytest assertion. v0.1.0 on PyPI + Homebrew + GHCR. |
...
| [ragvitals](https://github.com/MukundaKatta/ragvitals) | Python | Five-dimensional production drift detection for RAG: query distribution, retrieval relevance, embedding drift, response quality, judge drift. Library not platform — bring-your-own time-series store via `Sink` protocol. Constant-baseline-aware classifier so a perfectly stable system that suddenly drops still alarms. v0.1.0 (PyPI pending). |
| [ragvitals-gemma-demo](https://github.com/MukundaKatta/ragvitals-gemma-demo) | Python | Companion demo for the dev.to challenge series. Exercises ragvitals across a Claude↔Gemma 4 model swap (synthetic + real-models paths), a pgvector + pgai + Ollama stack, a self-auditing Postgres path, and a 60-line Redis Streams sink. CI green on Python 3.10–3.13. |
...
Archived originals and forks are indexed in [`data/repos.ndjson`](data/repos.ndjson) (one JSON record per line, 945 records total). Refresh with:
